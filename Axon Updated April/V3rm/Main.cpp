...
DWORD WINAPI input(PVOID lvpParameter)
{
	...
	DWORD dwRead;
	hPipe = CreateNamedPipe(TEXT("\\\\.\\pipe\\Axon"), // Used to connect to the pipe (you can change it to anything)
...
void main()
{
	    ConsoleBypass("Axon Updated | April 2021 | Updated by SpeedSterKawaii"); // Console Title.
		getdatamodeltesting();
		...
		MessageBoxA(NULL, "Axon Injected, Aero for Axon, Hexus for Bridge.h, SpeedSterKawaii for Updating, MellonYT for Ez Addresses.", "Axon has Injected.", MB_ICONINFORMATION);
}
